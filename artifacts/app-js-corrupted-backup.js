...
function syncAffiliatePresetActionState(containerId = "", platform = "") {
  ...
  if (checkBtn) {
    ...
    checkBtn.textContent = pending ? "Dang check..." : "Check";
  }
}
...
function buildAffiliatePresetMarkup(containerId) {
  ...
  if (!availablePresets.length) {
    return `
      <div class="affiliate-preset-shell empty">
        <div class="affiliate-preset-empty">
          ChÆ°a cÃ³ link affiliate máº·c Ä‘á»‹nh. VÃ o <span class="upgrade-link" onclick="openAccountSection('settings')">TÃ i khoáº£n â†’ CÃ i Ä‘áº·t</span> Ä‘á»ƒ lÆ°u sáºµn Shopee hoáº·c TikTok.
        </div>
      </div>`;
  }
  return `
    <div class="affiliate-preset-shell">
      <div class="affiliate-preset-head">
        <div>
          <strong>Link affiliate Ä‘Ã£ lÆ°u</strong>
          <span>Chá»n nhanh Ä‘á»ƒ Ä‘á»• vÃ o form, khÃ´ng cáº§n dÃ¡n láº¡i má»—i láº§n.</span>
        </div>
        <button class="btn-cp" type="button" onclick="openAccountSection('settings')">Chá»‰nh preset</button>
      </div>
      <div class="affiliate-preset-grid">
        ${availablePresets
          .map((preset) => {
            ...
            return `
              <div class="affiliate-preset-card">
                ...
                <div class="affiliate-preset-actions">
                  <span class="affiliate-preset-status ${health.tone}" id="${containerId}_presetStatus_${preset.key}">${esc(health.label)}</span>
                  <button class="btn-cp" type="button" id="${containerId}_presetUse_${preset.key}" onclick="useAffiliatePreset('${containerId}','${preset.key}')" ${usable ? "" : "disabled"}>DÃ¹ng link</button>
                  <button class="btn-cp" type="button" id="${containerId}_presetCheck_${preset.key}" onclick="checkAffiliatePreset('${preset.key}','${containerId}')" ${pending ? "disabled" : ""}>${pending ? "Dang check..." : "Check"}</button>
                </div>
              </div>`;
          })
          .join("")}
...
function ensureVideoPopupStageFields(containerId) {
  const videoTextInput = document.getElementById(`${containerId}_videotext`);
  if (!videoTextInput) return;
  if (document.getElementById(`${containerId}_video_popup_url_3s`)) return;
  const anchorBlock = videoTextInput.closest("div");
  ...
  ensureVideoPopupStageFields(containerId);
...
function renderTeamPage() {
  ...
  const domainLabel = document.getElementById("teamDomainLabel");
                ...
                <label class="fl" style="margin-bottom:3px">N?i dung CTA popup</label>
  if (seatHint) {
  ...
  if (workspaceStatus) {
    workspaceStatus.textContent = user
      ? `Seat đang dùng ${members.length}/${seatLimit} · Chủ workspace ${getUserDisplayName(user)}`
            Video autoplay ? popup 3s ? popup 5s ch?ng l?n ? popup 300s ? user b?m popup tr?n c?ng ?? m? App Shopee/TikTok.<br/>
  }
  if (ownerLabel) {
...
function setInlineUploadStatus(targetId, message = "", tone = "", progress = null) {
  ...
  el.textContent =
    progress === null ? message : `${message} ${Math.max(0, Math.min(100, Number(progress) || 0))}%`;
}
...
function uploadFormDataWithProgress(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onerror = () => reject(new Error("Loi ket noi"));
    xhr.onload = () => {
      let payload = {};
      try {
...
async function uploadImageFileToField(file, options = {}) {
  ...
  setInlineUploadStatus(statusId, "Dang upload anh...", "warn", 0);
  const data = await uploadFormDataWithProgress("/api/upload-image", fd, {
    onProgress: (progress) =>
      setInlineUploadStatus(statusId, "Dang upload anh...", "warn", progress),
  });
  ...
  setInlineUploadStatus(statusId, "Upload anh xong", "ok", 100);
...
async function handleVideoUploadLegacy(event, cid) {
  ...
  const area = document.getElementById(cid + "_vuploadarea");
  if (area) area.style.borderColor = "var(--brand)";
  setInlineUploadStatus(`${cid}_videoUploadStatus`, "Dang upload video...", "warn", 0);
  const fd = new FormData();
  fd.append("video", file);
  try {
    const data = await uploadFormDataWithProgress("/api/upload-video", fd, {
      onProgress: (progress) =>
    ...
    const urlInput = document.getElementById(cid + "_videourl");
    if (urlInput) {
      urlInput.value = data.url.startsWith("/")
        ? window.location.origin + data.url
        : data.url;
    }
    if (data.thumb) {
    ...
    toast(
      data.thumb
        ? "Upload xong! Thumbnail da tu dong lay tu video."
        : "Upload video thanh cong!",
      "ok",
    );
  } catch (error) {
    if (error?.status === 401) {
      redirectToAuth("login", "Cáº§n Ä‘Äƒng nháº­p Ä‘á»ƒ upload video.");
      return;
    }
    if (error?.status === 403 && error?.payload?.upgrade) {
      toast(error.payload.error || "Tinh nang nay yeu cau goi Pro", "warn");
    ...
    setInlineUploadStatus(
      `${cid}_videoUploadStatus`,
      error?.message || "Upload video that bai",
      "err",
    );
    toast(error?.message || "Upload video that bai", "err");
    if (area) area.style.borderColor = "var(--border2)";
  } finally {
    if (input) input.value = "";
  }
...
async function handleVideoUpload(event, cid) {
  ...
  try {
    ...
    setInlineUploadStatus(
      `${cid}_videoUploadStatus`,
      error?.message || "Loi upload video",
      "err",
    );
    toast(error?.message || "Loi upload video", "err");
    if (area) area.style.borderColor = "var(--border2)";
...
async function handleFileUpload(event, cid) {
  ...
  try {
    ...
    toast(data?.url ? "Upload anh thanh cong!" : "Upload anh xong!", "ok");
  } catch (error) {
    if (error?.status === 401) {
      redirectToAuth("login", "Cáº§n Ä‘Äƒng nháº­p Ä‘á»ƒ upload áº£nh.");
      return;
    }
    if (error?.status === 403 && error?.payload?.upgrade) {
      toast(error.payload.error || "Tinh nang nay yeu cau goi Pro", "warn");
    ...
    setInlineUploadStatus(
      ...
      error?.message || "Upload anh that bai",
      "err",
    );
    toast(error?.message || "Loi upload", "err");
  } finally {
    if (input) input.value = "";
  }
}
...
async function handleEditImageUpload(event) {
  ...
  try {
    ...
    toast(error?.message || "Khong the upload anh", "err");
  } finally {
    if (input) input.value = "";
  }
}
