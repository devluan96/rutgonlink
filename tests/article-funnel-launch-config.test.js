...
test("article funnel preview page treats TikTok popup 20s as inline launch instead of deeplink go routing", () => {
  const apiSource = fs.readFileSync(
    path.join(__dirname, "..", "api", "index.js"),
    "utf8",
  );
  ...
  assert.match(
    apiSource,
    /return \(\s+\(normalizedStageKey === "3s" && directPlatform === "shopee"\) \|\|\s+\(normalizedStageKey === "20s" && directPlatform === "tiktok"\)\s+\);/s,
  );
  assert.match(
    apiSource,
    /function shouldUseArticleFunnelDeeplinkRoute\(stage\) \{\s+return false;\s+\}/s,
  );
});
