...
async function main() {
  ...
  try {
    ...
    const { page: authedPage } = await registerAuthedPage(browser);
    try {
      ...
      await Promise.all([
        ...
        authedPage.click("#landingShortenConfirm"),
      ]);

      const authedGuestShorten = await shortenAsAuthedUser(authedPage, {
        url: "https://s.shopee.vn/4Axe4JRRDO",
      });
      assert.equal(authedGuestShorten.status, 428);
      assert.equal(authedGuestShorten.data.confirmationRequired, true);

      const authedConfirmedShorten = await shortenAsAuthedUser(authedPage, {
        url: "https://s.shopee.vn/4Axe4JRRDO",
        confirm_affiliate: true,
      });
      assert.equal(authedConfirmedShorten.status, 200);
      assert.ok(authedConfirmedShorten.data.short_url);
      ...
      assert.match(authedAppPrompt, /xác nhận/);

      await Promise.all([
        authedPage.waitForFunction(
          () => document.getElementById("createFormArea_res")?.classList.contains("show"),
        ),
        authedPage.click("#createFormArea_err .upgrade-link"),
      ]);
